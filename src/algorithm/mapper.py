...
class Mapper:
    def mapper(self, genome, tree):
        ...
        the params dict to ensure the correct type of individual is being
        created.
        ...
        phenotype, nodes, invalid, depth, used_codons = None, None, None, None, None
        ...
        if genome:
            ...
            # (prevents cross-contamination of information between
            # individuals).
            ...
            if params['GENOME_OPERATIONS']:
                ...
                phenotype, genome, tree, nodes, invalid, depth, \
                used_codons = self.map_tree_from_genome(genome)
    ...
    def map_ind_from_genome(self, genome):
        ...
        max_tree_depth, max_wraps = params['MAX_TREE_DEPTH'], params[
            'MAX_WRAPS']
    ...
    def map_tree_from_genome(self, genome):
        """
        ...
        output, used_codons, nodes, depth, max_depth, invalid = \
            self.genome_tree_map(tree, genome, [], 0, 0, 0, 0)
    ...
    def genome_tree_map(self, tree, genome, output, index, depth, max_depth,
                        nodes,
                        invalid=False):
        """
        ...
        if not invalid and index < len(genome) * (params['MAX_WRAPS'] + 1):
            ...
            # remaining codons in the genome, then we can continue to map the
            # tree.

            if params['MAX_TREE_DEPTH'] and (
            ...
            for symbol in chosen_prod['choice']:
                # Add children to the derivation tree by creating a new
                # instance of the representation.tree.Tree class for each
                # child.

                if symbol["type"] == "T":
                    # Append the child to the parent node. Child is a terminal,
                    # do not recurse.
                    ...
                    output, index, nodes, d, max_depth, invalid = \
                        self.genome_tree_map(tree.children[-1], genome, output,
        ...
        if not invalid:
            ...
            if params['MAX_TREE_DEPTH'] and (
                        max_depth > params['MAX_TREE_DEPTH']):
                # If our maximum depth exceeds the limit, the solution is
                # invalid.
                invalid = True
